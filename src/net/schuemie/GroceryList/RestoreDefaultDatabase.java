...
//	public String type=getIntent().getExtras().getString("Type");
							...
							if (tagName.equals("category")){
								...
								for (int i = 0; i < xmlParser.getAttributeCount(); i++){
									if (xmlParser.getAttributeName(i).equals("name")){
										...
										getContentResolver().insert(GroceryListContentProvider.CATEGORIES_URI, values);
										//Retrieve the auto-generated ID:
										Cursor cursor = getContentResolver().query(GroceryListContentProvider.CATEGORIES_URI, null, Categories.NAME +" = '"+categoryName+"'", null, null);
										cursor.moveToFirst();
										categoryID = cursor.getLong(cursor.getColumnIndex(BaseColumns._ID));
										break;
									}
								}
								...
								for (int i = 0; i < xmlParser.getAttributeCount(); i++){
									if (xmlParser.getAttributeName(i).equals("name")){
										...
										getContentResolver().insert(GroceryListContentProvider.ITEMS_URI, values);
										break;
									}
								}
							}
