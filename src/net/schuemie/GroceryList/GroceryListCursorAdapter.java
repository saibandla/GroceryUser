...
public class GroceryListCursorAdapter extends CursorAdapter {
    ...
    private void updateView(View view, Cursor cursor){
    	...
    	{
    		nameLabel.setTextColor(Color.GREEN);
    	//	amount_unitText.setTextColor(Color.WHITE);
    		System.err.println("nameeeeeeeeeeeeeeeeeeeeeeeeee = "+ cursor.getString(cursor.getColumnIndex(Items.NAME)));
    		
    	     System.err.println("vector size = "+vector.size());
    		double amountDouble = cursor.getDouble(cursor.getColumnIndex(Items.AMOUNT));
    		String amount;
    		if (amountDouble == 0)
    			amount = "";
    		else
    		...
    		if(vector.contains(cursor.getString(cursor.getColumnIndex(Items.NAME)) +" "+amount+" "+unit))
    		{
    			
    		}
    		else
    		{
    		vector.add( cursor.getString(cursor.getColumnIndex(Items.NAME)) +" "+amount+" "+unit);
    		}
    		if (!unit.equals("")){
