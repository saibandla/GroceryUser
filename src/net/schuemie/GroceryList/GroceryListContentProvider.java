...
public class GroceryListContentProvider extends ContentProvider{
    ...
    public int delete(Uri uri, String where, String[] whereArgs) {
        ...
        case ITEM_ID:
        	...
        	count = database.delete(ITEMS_TABLE_NAME, BaseColumns._ID + "=" + itemId + (!TextUtils.isEmpty(where) ? " AND (" + where + ')' : ""), whereArgs);
        	break;
        case CATEGORIES:
        ...
        case CATEGORY_ID:
        	...
        	getContext().getContentResolver().delete(ITEMS_URI, Items.CATEGORY + "=" + categoryId, null);
        	count = database.delete(CATEGORIES_TABLE_NAME, BaseColumns._ID + "=" + categoryId + (!TextUtils.isEmpty(where) ? " AND (" + where + ')' : ""), whereArgs);
    ...
    public int update(Uri uri, ContentValues values, String where, String[] whereArgs) {
        ...
        case ITEM_ID:
        	String itemId = uri.getPathSegments().get(1);
        	count = database.update(ITEMS_TABLE_NAME, values, BaseColumns._ID + "=" + itemId + (!TextUtils.isEmpty(where) ? " AND (" + where + ')' : ""), whereArgs);
        	break;
        case CATEGORIES:
            count = database.update(CATEGORIES_TABLE_NAME, values, where, whereArgs);
            break;
        case CATEGORY_ID:
        	String categoryId = uri.getPathSegments().get(1);
        	count = database.update(CATEGORIES_TABLE_NAME, values, BaseColumns._ID + "=" + categoryId + (!TextUtils.isEmpty(where) ? " AND (" + where + ')' : ""), whereArgs);
        	break;
        default:
            throw new IllegalArgumentException("Unknown URI " + uri);
        }       
    ...
    private class DatabaseHelper extends SQLiteOpenHelper {
        ...
        public void createDatabase(SQLiteDatabase db){
            db.execSQL("CREATE TABLE " + ITEMS_TABLE_NAME + " ("
            		...
            		+ Items.AMOUNT + " FLOAT,"
            		+ Items.UNIT + " TEXT,"
            		+ Items.ONLIST + " INTEGER,"
            		+ Items.CATEGORY + " INTEGER"
            		+");");
